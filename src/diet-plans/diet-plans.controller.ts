...
export class DietPlansController {
  constructor(private plans: DietPlansService) {}
  ...
  @Post()
  @Roles('ADMIN', 'TRAINER')
  create(@CurrentUser() user: User, @Body() dto: CreateDietPlanDto) {
    return this.plans.create(user.gymId, user.id, dto);
  }

  @Put(':id')
  @Roles('ADMIN', 'TRAINER')
  update(@CurrentUser() user: User, @Param('id') id: string, @Body() dto: UpdateDietPlanDto) {
  ...
  @Post('assignments')
  @Roles('ADMIN', 'TRAINER')
  assign(@CurrentUser() user: User, @Body() dto: CreateDietAssignmentDto) {
